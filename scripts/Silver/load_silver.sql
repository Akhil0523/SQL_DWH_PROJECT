...
BEGIN
    ...
    BEGIN
        -- Error Handling for MySQL: prints simple error messages
        SELECT '==========================================' AS '';
        SELECT 'ERROR OCCURRED DURING LOADING SILVER LAYER' AS '';
        SELECT 'Check server logs for error details.' AS '';
        SELECT '==========================================' AS '';
        ROLLBACK;
    END;
    ...
    SELECT
        prd_id,
        REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- Extract and normalize category ID
        SUBSTRING(prd_key, 7) AS prd_key,                      -- Extract product key (from 7th character)
        prd_nm,
        IFNULL(prd_cost, 0) AS prd_cost,                       -- Convert NULL product cost to 0
        CASE 
    ...
    SELECT 
        ...
        sls_prd_key,
        sls_cust_id,
        -- Convert order date: if 0 or not 8 chars, set NULL; else convert to DATE
        CASE 
        ...
        END AS sls_due_dt,
        -- Recalculate sales if value missing or incorrect
        CASE 
    ...
    SELECT
        ...
        CASE
            ...
            ELSE bdate
        END AS bdate,
        -- Normalize gender and handle unknown cases
        CASE
    ...
    SELECT
        ...
        -- Normalize and handle missing/blank country codes
        CASE
